...
program_name = "Web Adjuster v0.181 (c) 2012-13 Silas S. Brown"
...
define("redirectFiles",default=False,help="If, when not functioning as a \"real\" HTTP proxy, a URL is received that looks like it requires no processing on our part (e.g. an image or downloadable file that the user does not want converted), and if this is confirmed via a HEAD request to the remote server, then redirect the browser to fetch it directly and not via Web Adjuster. This takes bandwidth off the adjuster server, and should mean faster downloads, especially from sites that are better connected than the adjuster machine. However it might not work with sites that restrict \"deep linking\".") # (TODO: option to list sites that are OK for redirectFiles, and/or list sites that aren't and that we're willing to full-proxy for)
...
def convert_to_requested_host(real_host,cookie_host=None):
    ...
    if options.publicPort==80: port=""
    else: port=":"+str(options.publicPort)
    if options.default_site:
      n=0
...
def errExit(msg):
    ...
    try: logging.error(msg) # in case run from crontab w/out output (and e.g. PATH not set properly)
    except: pass
...
def main():
    ...
    if options.ip_query_url and options.ip_change_command:
        ...
        global ip_query_url2,ip_query_url2_user,ip_query_url2_pwd
        ip_query_url2 = options.ip_query_url2
        ip_query_url2_user=ip_query_url2_pwd=None
        if ip_query_url2:
...
redirectFiles_Extensions=set("pdf epub mp3 aac zip gif png jpeg jpg exe tar tgz tbz".split()) # TODO: make this list configurable + maybe add a "minimum content length before it's worth re-directing" option
...
class RequestForwarder(RequestHandler):
    ...
    def inProgress(self):
        ...
        self.set_status(200)
        self.add_header("Pragma","no-cache") ; self.add_header("Vary","*") ; self.add_header("Expires","Thu Jan 01 00:00:00 1970") ; self.add_header("Cache-Control","no-cache, no-store, must-revalidate, max-stale=0, post-check=0, pre-check=0") # don't ANYBODY even THINK about caching this!
        ...
        self.doResponse2(("""<html><head><title>File conversion in progress</title><meta name="mobileoptimized" content="0"><meta name="viewport" content="width=device-width"></head><body><h1>File conversion in progress</h1>The result should start downloading soon. If it does not, try <script><!--
document.write('<a href="javascript:location.reload(true)">refreshing this page</a>')
//--></script><noscript>refreshing this page</noscript>.%s%s<hr>This is %s</body></html>""" % (backScript,warn,serverName_html)),True,False)
    ...
    def serve_mailtoPage(self):
        uri = self.request.uri[len(options.mailtoPath):]
        if '?' in uri: addr=uri[:uri.index('?')]
        else: addr=uri
        addr = urllib.unquote(addr)
        body = self.request.arguments.get("body",None)
        if body and type(body)==type([]): body=body[0]
        subj = self.request.arguments.get("subject",None)
        if subj and type(subj)==type([]): subj=subj[0]
        r = [] ; smsLink = ""
        ...
        if body:
            ...
            if self.checkBrowser(options.mailtoSMS):
                ...
                if '&' in smsLink: smsLink="[Before sending this text, replace -amp- with an ampersand. This substitution has been done in case your phone isn't compliant with RFC 5724.] "+smsLink.replace('&',' -amp- ')
        ...
        self.doResponse2(('<html><head><title>mailto: link - Web Adjuster</title><meta name="mobileoptimized" content="0"><meta name="viewport" content="width=device-width"></head><body><h3>mailto: link</h3>This link is meant to open an email client.<br>%s<br><a href=\"mailto:%s\">Open in email client</a> (if set up)%s%s<hr>This is %s</body></html>' % ("<br>".join(r),uri,smsLink,backScript,serverName_html)),True,False)
    ...
    def serve_submitPage(self):
        ...
        if not txt:
            ...
            return self.doResponse2(("""<html><head><title>Upload Text - Web Adjuster</title><meta name="mobileoptimized" content="0"><meta name="viewport" content="width=device-width"></head><body style="height:100%%;overflow:auto"><form method="post" action="%s"><h3 style="float:left;padding:0px;margin:0px">Upload Text</h3><span style="float:right"><input type="submit"><script><!--
...
//--></script></body></html>""" % (options.submitPath,)),True,False)
        ...
        class R:
            code = 200
            headers = H()
        r=R() ; r.body="""<html><head><title>Uploaded Text - Web Adjuster</title><meta name="mobileoptimized" content="0"><meta name="viewport" content="width=device-width"></head><body><h3>Your text</h3>%s<hr>This is %s. %s</body></html>""" % (txt2html(txt),serverName_html,backScriptNoBr) # backScriptNoBr AFTER the server notice to save vertical space
        self.doResponse(r,[False]*4,False,False)

    def checkTextCache(self):
    ...
    def doReq(self):
        ...
        if self.handleFullLocation(): return
        ...
        if self.request.headers.get_list("Cookie"):
            ...
            def ours(c): # don't forward our own cookies upstream (may confuse some sites, especially if a site uses Web Adjuster their end)
                ...
                c = c[:c.index('=')]
                return (options.htmlonly_mode and c==htmlmode_cookie_name) or (options.cssName and c=="adjustCssSwitch") or (options.htmlFilterName and c=="adjustNoFilter") or (options.renderName and c=="adjustNoRender") or (options.prominentNotice and c=="_WA_warnOK") or (c==adjust_domain_cookieName and self.cookie_host())
            if options.upstream_guard:
              def maketheirs(c):
                if options.cssName: c=c.replace("adjustCssSwitch1","adjustCssSwitch")
                if options.htmlFilterName: c=c.replace("adjustNoFilter1","adjustNoFilter")
                if options.renderName: c=c.replace("adjustNoRender1","adjustNoRender")
                return c
            else: maketheirs = lambda x:x
        ...
        if not isProxyRequest and any(re.search(x,self.urlToFetch) for x in options.prohibit): return self.redirect(self.urlToFetch)
        try:
            self.connection_header = self.request.headers['Connection']
            del self.request.headers['Connection']
        except: self.connection_header = None
        if options.redirectFiles and not (isProxyRequest or any(converterFlags) or viewSource) and '.' in self.request.uri and self.request.uri[self.request.uri.rindex('.')+1:].lower() in redirectFiles_Extensions and not (options.bitrate and not options.askBitrate and self.request.uri[self.request.uri.rindex('.'):].lower()==".mp3"): self.sendHead()
    ...
    def sendRequest(self,converterFlags,viewSource,isProxyRequest,follow_redirects):
        ...
        http.fetch(self.urlToFetch,
                   ...
                   proxy_host=upstream_proxy_host, proxy_port=upstream_proxy_port,
    ...
    def doResponse(self,response,converterFlags,viewSource,isProxyRequest):
        ...
        if hasattr(self.request,"old_cookie"): self.request.headers["Cookie"] = self.request.old_cookie # put it back as we'll need to refer to our cookies
        if self.connection_header: self.request.headers['Connection'] = self.connection_header # put it back before writing the response
        ...
        if not response.code or response.code==599:
            ...
            error = """<html><head><title>Error</title><meta name="mobileoptimized" content="0"><meta name="viewport" content="width=device-width"></head><body><h1>Error</h1>%s<br>Was trying to fetch %s<hr>This is %s</body></html>""" % (error,ampEncode(tryFetch),serverName_html)
            self.set_status(504)
        ...
        for name,value in response.headers.get_all():
          ...
          if (do_pdftotext or do_epubtotext) and name.lower() in ["content-disposition","content-type"]: continue # we're re-doing these also
            ...
            if options.upstream_guard:
                if options.cssName: value=value.replace("adjustCssSwitch","adjustCssSwitch1")
                if options.htmlFilterName: value=value.replace("adjustNoFilter","adjustNoFilter1")
                if options.renderName: value=value.replace("adjustNoRender","adjustNoRender1")
        ...
        if do_pdftotext or do_epubtotext:
            ...
            self.set_header("Content-Disposition","attachment; filename="+urllib.quote(self.request.uri[self.request.uri.rfind("/")+1:self.request.uri.rfind(".")]+".txt"))
            ...
            def txtCallback(self,fn,cmdname,err):
                try: txt = open(fn+".txt").read()
                    ...
                    txt = "Could not read %s's output from %s\n%s\n(This is %s)" % (cmdname,fn+".txt",err,serverName)
                    try: open(fn+".txt","w").write(txt) # must unconditionally leave a .txt file as there might be other requests waiting on cache
                    except: txt += "\nCould not write to "+fn+".txt" # TODO: logging.error as well ?
                unlinkOutputLater(fn+".txt")
                unlink(fn)
        ...
        if (options.pdftotext or options.epubtotext or options.epubtozip or options.askBitrate or options.mailtoPath) and (do_html_process or (do_json_process and options.htmlJson)):
            # Add PDF links BEFORE the external filter, in case the external filter is broken and we have trouble parsing the result
            if do_html_process:
                adjustList.append(AddConversionLinks(options.wildcard_dns or isProxyRequest))
            else:
                ctl = find_HTML_in_JSON(body)
                for i in range(1,len(ctl),2):
                    ctl[i] = json_reEscape(add_conversion_links(ctl[i],options.wildcard_dns or isProxyRequest))
                body = "".join(ctl)
        if options.headAppendCSS:
          # remove !important from other stylesheets
          important = re.compile("! *important")
          if (do_html_process or (do_css_process and not self.urlToFetch == options.headAppendCSS and not (options.protectedCSS and re.search(options.protectedCSS,self.urlToFetch)))) and re.search(important,body):
    ...
    def headResponse(self,response):
        ...
        for name,value in response.headers.get_all():
          if name.lower()=="content-type":
            value=value.lower()
            might_need_processing_after_all = ("html" in value or "css" in value or "javascript" in value or "json" in value) # these need at least domain processing
...
def urlbox_html(htmlonly_checked):
    r = '<html><head><title>Web Adjuster start page</title><meta name="mobileoptimized" content="0"><meta name="viewport" content="width=device-width"></head><body><form action="/">Website to adjust: <input type="text" name="q"><input type="submit" value="Go">'+searchHelp()
        ...
        if tag=="a" and "href" in attrsD:
            ...
            if l.startswith("http://"):
                ...
                for k,v in items(attrs):
                    ...
                    r.append(k+'="'+v.replace('&','&amp;').replace('"','&quot;').replace('&amp;#','&#')+'"')
                r.append('>')
    ...
    def handle_endtag(self, tag):
        if tag=="a" and ((self.gotPDF and options.pdftotext) or (self.gotEPUB and (options.epubtozip or options.epubtotext)) or (self.gotMP3 and options.bitrate and options.askBitrate)):
            ...
            if self.gotPDF: linksToAdd.append("%s\"%s%s\">text</a>" % (linkStart,self.gotPDF,pdftotext_suffix))
                ...
                if options.epubtotext: linksToAdd.append("%s\"%s%s\">text</a>" % (linkStart,self.gotEPUB,epubtotext_suffix))
                if options.epubtozip: linksToAdd.append("%s\"%s%s\">zip</a>" % (linkStart,self.gotEPUB,epubtozip_suffix))
...
def add_conversion_links(h,offsite_ok):
    # (wrapper for when we can't avoid doing a special-case HTMLParser for it)
    return HTML_adjust_svc(h,[AddConversionLinks(offsite_ok)],can_use_LXML=False) # False because we're likely dealing with a fragment inside JSON, not a complete HTML document
    ...
    class Adjustment:
        ...
        def handle_starttag(self, tag, attrs):
            if tag==intag: self.intag=True
              ...
              attrsD = dict(attrs)
              if ("onclick" in attrsD and transformFunc(attrsD["onclick"]) != attrsD["onclick"]) or ("id" in attrsD and transformFunc(attrsD["id"]) != attrsD["id"]): # TODO: name as well? (shouldn't be needed for our own scripts)
    ...
    if options.headAppendCSS:
        ...
        if options.cssName:
          ...
          if slow_CSS_switch:
              ...
              if useCss:
                  headAppend += '<link rel="stylesheet" type="text/css" href="%s"%s>' % (options.headAppendCSS,link_close)
                  if options.cssHtmlAttrs:
                      i=html.lower().find("<body")
                      if i>-1:
                          i += 5 # after the "<body"
                          html = html[:i] + " " + options.cssHtmlAttrs + html[i:]
          else: # client-side only CSS switcher:
            headAppend += """<link rel="alternate stylesheet" type="text/css" id="adjustCssSwitch" title="%s" href="%s"%s>""" % (cssName,options.headAppendCSS,link_close)
...
//--></script>""" % (detect_iframe,cssName,cookieHostToSet,cookieExpires,cookieHostToSet,cookieExpires,cookieHostToSet,cookieExpires,cookieHostToSet,cookieExpires) # (hope it helps some MSIE versions to set cookies 1st, THEN scroll, and only THEN change the document. Also using onclick= rather than javascript: URLs)
        ...
        else: headAppend += '<link rel="stylesheet" type="text/css" href="%s"%s>' % (options.headAppendCSS,link_close)
...
//--></script>""" % (detect_iframe,adjust_domain_cookieName,adjust_domain_none,cookieExpires,cookieHostToSet,cookieHostToSet) # (we should KNOW if location.href is already that, and can write the conditional here not in that 'if', but they might bookmark the link or something)
    ...
    if options.headAppendRuby: bodyPrepend += """
...
v=navigator.userAgent.slice(wk+7,wk+12);if(v>=534.3&&v<535.7&&document.readyState!='complete')document.write('<style>rt{padding-left:1ex;padding-right:1ex;}<\/style>')}
//--></script>
...
class Dynamic_DNS_updater:
    ...
    def queryLocalIP(self):
        ...
        if ip_query_url2_user:
            ...
            callback = lambda r:AsyncHTTPClient().fetch(ip_query_url2, callback=handleResponse, auth_username=ip_query_url2_user,auth_password=ip_query_url2_pwd)
    ...
    def newIP(self,ip):
        ...
        def runCmd(cmd):
            ...
            os.system(cmd)
            helper_thread_count -= 1
        threading.Thread(target=runCmd,args=(cmd,)).start()
