...
# Note: start_multicore() does NOT rely on SO_REUSEPORT multiplexing: it forks AFTER we've started listening to the port, and processes share the SAME socket.  TODO: can overloaded subprocesses call TCPServer.stop() and TCPServer.start() to pause handling new connections for a particular httpserver / port (e.g. if options.js_429 is set).
