...
if android=="file:///android_asset/index.html":
  # generic browser: add bookmarks function. TODO: mention in the --android help text that we do this? + where to say you can put android:sharedUserId in AndroidManifest.xml to share bookmarks with another app if signed by same cert (but does this even work? seems you have to call createPackageContext on the other package name and call getSharedPreferences on that, AND make it MODE_WORLD_READABLE, which is deprecated in API 17 and might raise SecurityException in Android N, instead of 0.  If can't get it to work, don't change SharedPreferences to Preferences, as that would break existing bookmarks)
