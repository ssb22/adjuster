...
ssh freebsd 'cd /usr/ports/www/adjuster/ && rm -f distinfo && make makesum && portlint -A && git add * && git commit * -m "www/adjuster '"$(grep -m 1 '^program_name' ../adjuster.py|cut -d ' ' -f5)"'" && git format-patch --stdout -1' > adjuster.mbox &&
