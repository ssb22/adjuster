...
if [ "$(uname -s)" = "FreeBSD" ] ; then
    ...
    if ! [ -f /usr/ports/Mk/bsd.port.mk ] ; then rm -rf /usr/ports; git clone --depth 1 https://github.com/freebsd/freebsd-ports /usr/ports; fi # use the mirror to save upstream bandwidth: we're not going to push from here
